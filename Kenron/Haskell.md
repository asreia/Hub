...
- 関数をカリー化するとは、関数が必要とする**引数の数より少ない数の引数を渡す**と、残りの引数を必要とする**関数を返す関数**にすることである(関数型ではこれが一般的)
...
8. `g`が引数に`a`を取り、もう一つ引数を取る関数を返し、`f`が引数に`(g a)の返された関数`を引数に取り値を返す(`g`は非関数型でいうコールバック)
    `f (g a) ❰f:1,g:2,a:0❱`
9. 内部の`(an1 an2..ann)`が値または関数を返し外部の`(b11..b1n)`が内部の`(an1 an2..ann)`を引数に取り値または関数をまた外部に返す
    `(..(b11..(a11 a12..a1n)..(a21 a22..a2n)..b1n..)..) ❰ann,bnn:0個から任意個の引数を取る❱`
10. `9.`をランダムに再帰的に拡張すると以下の様になり式というのは本当に**数学の数式と構造が同じ**である(ラムダ式、if,case式は関数の`=`の左側に追いやることができる。where,let,do記法は展開できる)
...
`(a,_,c) = (1,2,3)` => `a = 1, c = 3`
6. asパターン`@`への束縛
`all@(_,_,_) = (1,2,3)` => `all = (1,2,3)`

#### パターンマッチ

- パターンマッチは、関数に渡さっる値により評価する式を変えることによって**処理を分岐**します。(必ず値を返すswitch文(C#のswitch式)に近い構文です)
...
-- Int ∈ Interface と見ることもできます(値(1,2,3..) ∈ Int ∈ Interface ⊂ Show みたいな)
...
-- test1 :: (Num a, Integral b) => a -> b -> b -- 期待された型 'a' と実際の型 'b' をマッチングできませんでした。
    ...
    -- この場合、"型クラス1"は"型クラス2"と同じ集合の型しか選択肢が無く"型クラス1"を"型クラス2"の差集合を取った場合、確実に"+"演算子に対して型が唯一に定まらずエラーとなる
test1 :: Num a => a -> a -> a
test1 n m = n + m
...
6. Haskellは**命名規則が厳しく**、型と値は大文字から、多相変数と引数は小文字から、中置記法の演算子は記号文字、で書くよう制約している。
    そのおかげで、`(),{},.,:`などの構文を入れなくても文字を並べただけで読みやすくなっている(型と値は`data`で定義するので構文解析は無関係にできるかな)
...
10. haskellの関数は**全public全static、オーバーロード不可**、関数は状態の更新を恐れる必要がない
...
ノードベースプログラミングは関数型
![Visual_Scripting](Visual_Scripting.png)
...
- 関手と自然変換
  ...
  - 直積、直和
    ...
    ポリモーフィズム、型(Intとか)の値を直和と見る。↑**C#で代数的データ型(data)** を表現してみる試み
      - ifなどの分岐は、直和の場合分け。
      (*)とか(+)とかの演算子は、最終的に**ALU**による場合分け(ALUへの制御信号({\*,/,+,-}なら2^2=4本)と入力データ(8bitCpuなら2項演算なので2\*8=16本)によるパターンマッチ)
- 関数型とオブジェクト指向の違い
  - **`=`の意味の違い**。何であるか?何をするか?
...
- 圏論とは、なんらかの**2つの対象**に何らかの**繋がり**がある時それを**射**と呼び、その射が2つ繋がるとき**単位律、推移律、結合律**を満みたし、その**射の集まりを圏**と呼び、
その射の繋がり方、**射の性質**、圏の構造、圏と圏の関係を議論するのが圏論である(で、い、いいかな?..)

- 圏とは、射と射の繋がり方が**ある法則を満たしている空間**である
  - その法則とは、集合論の写像、順序関係の`<=`、SNSのフォロー関係、など、そこに**矢印的な何かの集まり**が、単位律、推移律、結合律、を満たせばその集まりを圏として定義できます(集合の中身が矢印の繋がりになった様なもの)
