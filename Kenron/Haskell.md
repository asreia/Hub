...
4. **型クラス**によるパターンマッチはできない
    ...
    -- f4 True = show True  -- 同じShowクラスのインスタンスであっても、型が違うので"a"がどっちの型で定義すればいいか分からない?
                            -- これをしたい場合、"f4"を"Show"型クラスのメソッドとして追加し、をそれぞれの型(IntとBool)でオーバーライドする必要がある(IntとBoolを混ぜることはできない)
    ```
...
return x = State $ \s -> (x, s)                -- ↓"h"の中に"s -> (a, newState)"が畳み込まれている?
...
- {class=> m = f2 f1 a <=instance} : m    //**複数の文脈**(と型)が**一つの型クラス**と表すこともある
...
- 圏論とは、なんらかの**2つの対象**/オブジェクトに何らかの**繋がり**があり、**その繋がりが2つ**あるときその関係性が、**反射律、推移律、結合律**、を満たすとき、**その繋がりを射**と呼び、その**射の集まりを圏**と呼び、
その射の繋がり方、**射の性質**、圏の構造、圏と圏の関係を議論するのが圏論である(で、い、いいかな?..)

- 圏とは、射と射の繋がり方が**ある法則を満たしている空間**である
  - その法則とは、集合論の写像、順序関係の`<=`、SNSのフォロー関係、など、そこに**矢印的な何かの集まり**が、反射率、推移律、結合律、を満たせばその集まりを圏として定義できます
...
- Hask圏
  - 圏の要素など
    ...
    - 関手
      - 対象: 型コンストラクタ
      - 射: Functorの`fmap`
    - 自然変換: 型が`m a`となっている時、射が文脈(`m`)だけに依存する射
    - 終対象: ユニット型`()`と同型な型全て
    - 始対象: 値が一つも無い型?!
    - 直積への射を作る関数: `(&&&)`
    - 直和への射を作る関数: `(|||)`
  ...
  - 圏論の関心事?
    ...
    - 自然変換という、**内部の型に依存しない射**をどう作るか?(モナドなど)
