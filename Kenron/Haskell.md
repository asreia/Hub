...
4. **型クラス**によるパターンマッチはできない
    ...
    f4 :: Show a => a -> String
    f4 a = show a
    -- f4 1 = show 1    -- この2つを同時に定義することはできない -- 追記: どっちも定義できない(aのみ通る)。
...
- Hask圏
  - 圏の要素など
    ...
    - 関手
      - **対象: 型コンストラクタ**
      - 射: Functorの`fmap`
    - 自然変換: 型が`m a`となっている時、関数が文脈(`m`)だけに依存する関数 (対象を関手先の対象から関手先の対象への射に対応付ける。その射を全て集めたものが自然変換)https://youtu.be/IpvhsTpsULk?t=192
