...
- CLI(Common Language Infrastructure): .netを中心に**多言語を多プラットフォーム**で動くようにした**仕様**。ECMA-335で標準化されている
  ...
  - 以下はCLIの実装部分、各プラットフォームにインストールされるもの  
    - BCL(Base Class Library): 開発者が書いたコードを各プラットフォームで動くようにした**ライブラリ**(ILとネイティブ？)
    - VES(Virtual Execution System): 開発者のILとBCLを使って実際のプラットフォーム用の**ネイティブコードを生成**する
...
- ILスタックには**int32, int64, native int, F型, O型, ポインタ型(native unsigned int(*), &)** しか区別できない(ILスタックは.maxstack ⟪～⟫の事)  
...
- マネージ型
  ...
  - O型(IL:class, string, int32[],..)
    - **C#の参照型**である。(classまたは、struct(値型)の**ボックス化表現**(つまりclass))
    - **全ての領域に存在**できます
    - **O型は、ヒープへの参照**しか持ちません
    - 参照として**安全ではない事はできません**
    - **nullになります**
    - GC(Garbage collection)
      - **マネージ型**はCLR(.net)の管理によって**参照先(ヒープ)のオブジェクトが移動**(コンパクション)されうります
  ...
  - マネージドポインタ(IL:&)
    - **C#のref**である
    - **メソッドスタックのみ存在**できます (メソッドスタックは関数スタックの事です)
    - **マネージドポインタを指せない**それ以外は全て指せる(O型もアンマネージ型も指せる)
    - **nullにならない**
...
- アンマネージ型
  ...
  - 値型(IL:valuetype, int32, bool,..)
    - **C#の値型**である(C#の構造体(struct))(列挙型(enum)はC#的には構造体でないかも知れないが、ILではvaluetypeで変数定義されているので一応構造体の様なものだがフィールドメンバはstaticしか無い)
    - **全ての領域に存在**できます
  ...
  - アンマネージドポインタ(IL:*)
    ...
    - **全ての領域に存在**できる
    - **アンマネージ型**のみ指せます
    - O型の参照先である**ヒープのフィールド**のアンマネージ型か、静的領域(**staticフィールド**)のアンマネージ型を参照する場合は**fixedステートメント**が必要
    - **nullになります**
    - **unsafeキーワード**をブロックに付けなければ使えません
    - C++のような**ポインタ操作**ができます(安全ではない)

- 説明した全ての型で**生成される領域**は、メソッドの中で定義すると**メソッドスタックに生成**し、参照型(O型)(class)のフィールドに定義すると**ヒープに生成**される
- **IL**で、C#のクラス(class)と構造体(struct)を識別する方法は、**変数の宣言時**に、`valuetype ｢型名｣ ｢変数名｣` となっていたら**構造体(値型)**、
  `class ｢型名｣ ｢変数名｣` となっていたら**クラス(O型)** となっていて、型の定義時に決めるのではなく、**変数の宣言時に決める**ようになっている
  - C#の`static class`は、**変数の宣言**が出来ないので、ILでは**クラス(O型)** でも**構造体(値型)** でも無い
...
　~底位アドレス~    (この図は感で書いている)
 | プログラム  |                                  ＄Stack＝❰メソッドスタック❱ |`````````|        //refとO型(マネージドポインタ(安全))の使われ方を表した図
 |    定数    |                                 ref値型┌--[↑∫Stack∫]--ref--|->{field}|        //∫Stack∫はMainからメソッドスタックが積まれる
 |  静的変数  |                                        └->[↑∫Stack∫]--O型->|  Heep   |--┐O型  //Heepは[∫Stack∫]と[Static]から"大体"、O型によって参照される。
↓|   ヒープ   |                                     ref┌--[↑∫Stack∫(Main)] |         |<-┘        //参照されなくなるとGCの対象になる
↓|   ヒープ   | {マネージドヒープ(GCの管理下にある)}      └->[Static]---O型-->|_________|        //ref値型は自分より上のプッシュされた[∫Stack∫]を参照できない(O型はできる)
↓|   ヒープ   |                                                                                  //refは何らかのfieldも参照する事ができる
 |    ...     |                                     
 |    ...     |メ {"--繰り返し--"で繰り返されるスタックをメソッドスタックと呼ぶことにする。ILコードの .maxstack ⟪～⟫ が使うスタックをILスタックと呼ぶことにする}
...
↑| ILスタック  |ド {ILスタックは.maxstack ⟪～⟫ で容量が決められているので固定量}
...
| 具体的なIL命令書式(引数も含める)                                        |
| :---------------------------------------------------------------------- |
| `ldc⟪❰｡｡❰.i4❱＠⟪｡❰.⟪0～8⟫❱¦.m1¦.s num¦ num｡⟫｡｡❱｡¦｡❰⟪.i8¦.r4¦.r8⟫ num❱⟫` |
