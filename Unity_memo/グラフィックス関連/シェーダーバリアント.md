...
☆←検索。ここでは`Keyword`⇔`ShaderKeyword`

## シェーダーバリアントの物理的意味

- ☆**各ShaderStage**は、**入出力セマンティクス**で繋がれ **#define配列**を定義する。
...
- `_local`**サフィックス**: `LocalKeyword[] material.enableKeywords`**のみ**から**設定可能**。
    (`GlobalKeyword[] Shader.enableGlobalKeywords`からは**設定不能**にする) (`_local`が付いた`Keyword`は、`LocalKeyword.isOverridable`を`false`にする)
