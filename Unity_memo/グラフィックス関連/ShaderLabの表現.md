        ...
        ⟦∫LRetInd∫┃～⟧❰｡｡∮PROPERTY_ATTRIBUTE∮ ｢PropertyName｣ (｡"｢PropertyShowName｣" ∮PROPERTY_TYPE∮｡) = ∮PROPERTY_LITERAL∮｡｡❱
            ...
            ＃PROPERTY_ATTRIBUTE＝≪○¦PT⟪∮Int_ATT∮¦∮Float_ATT∮¦∮Range_ATT∮¦∮Color_ATT∮¦∮Vector_ATT∮¦∮2D_ATT∮¦∮Cube_ATT∮¦∮3D_ATT∮⟫≫
                ...
                ＃Float_ATT＝≪∮ALL_ATT∮∮Int_Float_ATT∮＠❰\[Gamma]『`Color Space:Linear`のときリニア変換される』❱≫
                    ＃Int_Float_ATT＝≪＠❰\[Enum＠❰(｡｡⟪｢C#列挙型｣¦｡⟦,┃1～7⟧❰｢ShowEnum｣,⟪～⟫❱｡⟫｡｡)❱]❱⏎『これはShaderKeyword設定不可
                                    ＠❰[｡Toggle＠❰(｢define ShaderKeyword｣)❱｡]❱⏎『←↓ShaderKeyword設定可 (MaterialPropertyDrawerなので、`material.SetFloat(..)`では**設定されない**ので注意(Inspectorからのみ))
                                    ＠❰[KeywordEnum(⟦, ┃1～⟧⟪｢Keyword｣¦None『Keyword無し版』⟫)]『｢PropertyName｣_｢Keyword｣(を大文字化)というShaderKeywordがdefineされる』❱≫
