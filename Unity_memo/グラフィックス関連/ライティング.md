...
- Lighting //draw.ioの中で球体を表示し矢印で示す(* occ, + emit, など)
  - 式
    ...
    未解決3D数学: 光の計算(BRDFなど,カンデラ,ルーメン,..,dx12本), 球面調和関数(SH), SDF, 光の屈折(コースティクス), スキニング, モーフィング, clothシミュ, シェーダー芸, 波動関数, 誘導弾
...
- 変える? **(変数の種類⟪_¦.¦Ø⟫Annotation(⟪＠⟦Object⟧＠⟦Value⟧⟫(自由))Space_AnnoElement)**この形式を守れば後は自由
