...
- `Shader "Custom/LayoutBlitShader"`
    ...
    Shader "Custom/LayoutBlitShader"
    {
        Properties
        {
            ...
            [Space(10)]
            [KeywordEnum(SAMPLE, LOAD)] _USE ("Use Sample or Load", Float) = 0
        }
        SubShader
        {
            Pass
            {
                ...
                #pragma shader_feature _USE_SAMPLE _USE_LOAD
    ...
    ```
- `RenderGraphTestPipline : RenderPipeline`
    ```csharp
    using System;
    ...
    ```
- `Feature : ScriptableRendererFeature`
    ```csharp
    using UnityEngine.Rendering;
    ...
    ```
- `Runtime\Utilities\Blit.hlsl`
    ```c
    //Shader.PropertyToID("_ZWrite"); //ZWrite[_ZWrite]
    struct Attributes
    {
    ...
    Varyings Vert(Attributes input)
    {
        ...
        output.texcoord   = _BlitScaleBias.zw + uv * _BlitScaleBias.xy; //展開した
